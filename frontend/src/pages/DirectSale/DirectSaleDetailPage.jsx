...
export default function DirectSaleDetailPage() {
    ...
    async function handleAddToCart() {
        ...
        try {
            ...
            await apiClient.postByHref(addToCartHref, {
...
/* Duplicate component accidentally pasted below; kept commented temporarily.
import { useEffect, useState, useContext } from 'react';
    ...
    export default function DirectSaleDetailPage() {
        ...
        async function loadDirectSale() {
            try {
                ...
                const directSaleData = await apiClient.getDirectSaleById(directSaleId);
                const itemId = directSaleData?.itemsId?.[0] ?? directSaleData?.itemIds?.[0];
        ...
        const endDate = directSale.endDate;
        const status = directSale.status ?? 'ACTIVE';
        const addToCartHref = directSale?._links?.addToCart?.href ?? null;
