...
import { IconChevronDown, IconChevronUp, IconSelector, IconSearch, IconEye, IconEyeOff, IconPlus, IconTrash } from '@tabler/icons-react';
import {
    Center,
    Group,
    ActionIcon,
...
const data: RowData[] = [
    { name: 'Fantasy Favourites', genre: 'Fantasy', visibility: 'public', sharedUntil: 12 },
    { name: 'Secret Reads', genre: 'Mystery', visibility: 'private', sharedUntil: null },
    { name: 'Sci-Fi Shelf', genre: 'Science Fiction', visibility: 'public', sharedUntil: 3 },
    { name: 'Classic Literature', genre: 'Classic', visibility: 'private', sharedUntil: null },
    { name: 'Lista fixe', genre: 'Classic', visibility: 'private', sharedUntil: null }
];
...
export function TableList() {
    ...
    const rows = sortedData.map((row) => (
        <Table.Tr key={row.name}>
            ...
            <Table.Td w={120}>
                <Center>
                    <ShareListModal listName={row.name} visibility={row.visibility} />
                </Center>
            </Table.Td>
            <Table.Td w={50}>
                <Center>
    ...
    return (
        <ScrollArea>
            ...
            <Table horizontalSpacing="md" verticalSpacing="xs" miw={900} layout="fixed">
                <Table.Thead>
                    <Table.Tr>
                        <Th width="25%" sorted={sortBy === 'name'} reversed={reverseSortDirection} onSort={() => setSorting('name')}>
                            List Name
                        </Th>
                        <Th width="15%" sorted={sortBy === 'genre'} reversed={reverseSortDirection} onSort={() => setSorting('genre')}>
                            Genre
                        </Th>
                        <Th width="15%" sorted={sortBy === 'sharedUntil'} reversed={reverseSortDirection} onSort={() => setSorting('sharedUntil')}>
                            Shared Until
                        </Th>
